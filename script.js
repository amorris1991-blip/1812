...
` : '';
  ...
  shuffledOptions.forEach((option, optIndex) => {
    ...
    optionDiv.setAttribute('data-correct', option.correct); // Store if correct
    ...
    // Pass the optionDiv to the answer function so we can mark it wrong
...
function answerCurrentQuestion(isCorrect, clickedOptionDiv) {
  ...
  if (isCorrect) {
      ...
      // Add continue button to quiz container
      document.getElementById('quiz-container').appendChild(continueBtn);
      ...
      feedbackDiv.innerHTML = gameState.quizMessage;
      document.getElementById('quiz-container').insertBefore(
          feedbackDiv, 
          document.getElementById('quiz-container').firstChild
      );
      
  } else {
      // WRONG! Apply penalty and mark the wrong answer
      ...
      // Mark this option as wrong (red border and disabled)
      clickedOptionDiv.style.border = '3px solid #f44336';
      clickedOptionDiv.style.background = 'rgba(244, 67, 54, 0.2)';
      clickedOptionDiv.style.pointerEvents = 'none';
      clickedOptionDiv.style.opacity = '0.5';
      
      // Re-render to show the error message at top
      renderCurrentQuizQuestion();
  }
}
...
function showReflectionScreen() {
  ...
  if (resourcesAbove30 === 5 && morale > 60) {
      victoryStatus = '🎖️ LEGENDARY - You crushed it! America survived with strength!';
      victoryClass = 'complete-victory';
  } else if (resourcesAbove30 >= 3 && morale > 40) {
      victoryStatus = '⚔️ SURVIVED - You made it through, but it was rough.';
